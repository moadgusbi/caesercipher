...
function encrypt() {
    ...
    let solved = "";
    let shift1 = parseInt(document.getElementById('shiftInput1').value);
    let shift2 = parseInt(document.getElementById('shiftInput2').value);
    
    let halfLength = Math.ceil(plainInput.length / 2);
    ...
    for (let i = 0; i < plainInput.length; i++) {
        ...
        if (char === ' ') {
            ...
            let shift = i < halfLength ? shift1 : shift2;
...
function decrypt() {
    ...
    let halfLength = Math.ceil(encryptedInput.length / 2);
